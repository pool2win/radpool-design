...
contrat. However, as we saw in the previous section the miner wants to
keep getting paid by the MSP on a regular basis as it keeps producing
...
contracts repeatedly. They also note that off-chain scaling for the LN
like use case, where one party pays the other can be scaled off-chain
by the transaction revocation technique used in
https://github.com/lightning/bolts/blob/master/03-transactions.md#revocationpubkey-derivation[Bolt #3].

In our case, the MSP is always paying the miner. In other words, the
MSP is the only one that can benefit from broadcasting an old
state. This makes the transaction revocation technique simpler to
apply than even in Lightning Network.
...
to move miner payout DLCs into LN contracts. We leave this
optimisation out from this initial proposal as it is a well understood
technique, mentioned in the initial DLC paper.

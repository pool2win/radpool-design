...
PPLNS whenever the pool finds a block. Meanwhile, miners are paid when
their DLC contracts expire.
...
.Timeline for Payouts to MSP and miner
image::payout-mechanism/cet-timeline.png["Timeline for Payouts to MSP and miner"]


The image shows an exaple situation where the pool finds blocks more

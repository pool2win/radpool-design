= Radpool - Decentralised Mining Pool
...
the centralised single entity into a federation running the pool. This
...
1. Block template construction is decentralised as each federation
member build an independent blocktemplate.
2. Share accounting is transparent, so the pool is fully auditable.
3. Federation members earn a yield in exchange of locking liquidity to
...
federation members, thus making the solution non-custodial.
